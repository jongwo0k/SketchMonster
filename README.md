...
확률에 따라 등급이 부여되고 등급, 종류, 스케치 과정에 따라 능력치를 부여합니다.
- Grade: S/A/B/C (10/30/40/20)
- Class: Bird(빠른 Speed, 낮은 HP), Dog(밸런스 형), Fish(느린 Speed, 높은 HP)
- Sketch: 많은 획으로 그려진 스케치를 통해 생성된 캐릭터는 높은 HP 보너스 스탯, 빠른 시간(많은 Remain Time)안에 그려진 스케치를 통해 생성된 캐릭터는 높은 Speed 보너스 스탯

최종 능력치 = Class별 기본값 X 등급 계수 + 추가 능력치(remain time || stroke count X 추가 계수)
...
## 오브젝트 관리 최적화 (예정)
Class별 스킬 추가, 자동 공격 전환, 공격 투사체 증가로 게임성(리젠률, 타격감, 속도감 등)을 높이려면 오브젝트 처리에 최적화가 필요하다. 
